...
- (void)updateAsynchronously
{
    ...
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        for (NSUInteger i = self.viewIndex; i < [self.views count]; i++)
        {
            ...
            {
                self.updating = YES;
                snapshot = [view snapshotOfSuperview:view.superview];
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                        blurredImage = [snapshot blurredImageWithRadius:view.blurRadius
            ...
            {
                [NSThread sleepForTimeInterval:view.updateInterval];
            }
